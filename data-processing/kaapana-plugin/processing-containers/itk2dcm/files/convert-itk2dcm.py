...
def make_out_dir(series_uid, study, case, segmentation=False, human_readable=False):
    ...
    if human_readable:
        case_dir = Path(batch_output)/study/str(case).split(".")[0]
    else:
...
class Nifti2DcmConverter:
    ...
    def __call__(self, path, meta_data=None):
        ...
        TODO: The segmentation files should be optional.

        {
            'Study UID': study_uid,
            'Study ID': some_study_id,
        ...
        The fields 'study_tags' and 'series_tags' can be used to to set arbitrary Dicom Tags on a 'per study' or 'per series' level. In case of conflicting tags, the more 
        specific case (i.e. the series-level value) is used.

        If any of these attributes is missing, the respective field will be filled with a default value for dicom.
        ...
        NOTE: This is not the meta.json file genrated for the conversion of segmentations and also not the meta json file generated by the collectMetaData workflow.
        TODO: Maybe make it compatible with collectMetadata json files, but if you have the dicom to collect the metadata from, why use this converter in the first place?

        """
        studys = next(os.walk(path))[1]

        for study in studys:
            if study != "BMC":
                continue
            try:
                with open(os.path.join(f"/{path}",study,"meta_data.json"), "r") as meta_file:
                    meta_data = json.load(meta_file)
            except FileNotFoundError:
                meta_data = {}
            self.convert_study(Path(path) / study, meta_data)

    def convert_study(self, path, meta_data={}):

        cases = self.parser(path, log='Info')

        study_instance_UID = meta_data.get("Study UID") if meta_data.get("Study UID") else generate_uid()
        study_id = meta_data.get("Study ID") if meta_data.get("Study ID") else str(path).split("/")[-1]
        patients = meta_data.get("Patients") if meta_data.get("Patients") else "all_different"
        series_descriptions = meta_data.get("Series Descriptions") if meta_data.get("Series descriptions") else ["" for _ in range(len(cases))]
        ...
        for i, case in enumerate(cases):
            ...
            series_tag_values["0020,000d"] = study_instance_UID
    ...
    def convert_series(self, case_path, study_id, patient_id, series_tag_values, seed='42', segmentation=None, seg_args=None, seg_info_path=None, *args, **kwds):
        """
        ...
        """
        series_id = str(case_path).split('/')[-1]
        series_description = kwds.get("series_description")
        if series_description == None:
            series_description = f"{study_id}-{series_id}-{patient_id}"
        ...
        series_instance_UID = kwds.get("series_uid") or generate_uid(entropy_srcs=[study_id, series_id, seed])
        out_dir = make_out_dir(series_instance_UID, study=study_id, case=series_id, segmentation=segmentation, human_readable=False)
